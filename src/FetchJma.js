class FetchJma {
  constructor() {
    const endpoint = "https://www.jma.go.jp/bosai/forecast/data";
    this.areaId = "130000";
    this.districtId = "130010";
    // 天気概況エンドポイント
    this.weatherOverviewEndpoint = `${endpoint}/overview_forecast`;
    // 天気予報エンドポイント
    this.weatherForecastEndpoint = `${endpoint}/forecast`;

    this.weathrStrIndex = 3;
    this.weatherCode = {
  ...
  getJsonFileName() {
    return `${this.areaId}.json`;
  }

  fetchWeatherJson() {
    const url = `${this.weatherForecastEndpoint}/${this.getJsonFileName()}`;
    this.weatherJson = JSON.parse(UrlFetchApp.fetch(url).getContentText());
    console.log("天気予報JSON: ", this.weatherJson);
  }

  findAreaFromTimeSeries(timeSeries, districtId) {
    return timeSeries.areas.find((item) => item.area.code === districtId);
  }

  getRainfallProbability() {
    const targetTimeSeries = this.weatherJson[0].timeSeries[1];
    const timeDefines = targetTimeSeries.timeDefines;
    const pops = this.findAreaFromTimeSeries(
      targetTimeSeries,
      this.districtId
    ).pops;
    const rainfallProbabilittyInfos = timeDefines.map((dateStr, i) => {
      // const 範囲終了日付
    ...
    return rainfallProbabilittyInfos;
  }

  fetchWeatherOverview() {
    const url = `${this.weatherOverviewEndpoint}/${this.getJsonFileName()}`;
    const json = JSON.parse(UrlFetchApp.fetch(url).getContentText());
    console.log("元気概況JSON: ", json);
    const weatherOverview = json.text.replace(
      /[ 　]|伊豆諸島では.+?。|\n\n\【関東甲信地方\】.+/gs,
      ""
    );
  ...
  getWeeklyWeatherForecast() {
    const json = this.weatherJson[1].timeSeries[0];
    const timeDefines = json.timeDefines;
    const areaInfo = this.findAreaFromTimeSeries(json, this.districtId);
    const pops = areaInfo.pops;
    const weathers = areaInfo.weatherCodes.map(
      (code) => this.weatherCode[code][this.weathrStrIndex]
    );
    const reliabilities = areaInfo.reliabilities;
